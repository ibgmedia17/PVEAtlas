...
export function KnowledgeGraph({ model, rootId, onSelect, theme }: { model: Topology; rootId: string; onSelect: (entity: Entity) => void; theme: 'atlas' | 'sakura' }) {
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({ container: container.current, elements, minZoom: .2, maxZoom: 3, wheelSensitivity: .12, userPanningEnabled: true, userZoomingEnabled: true, autoungrabify: false, boxSelectionEnabled: false, style: [
      { selector: 'node', style: { label: 'data(displayLabel)', width: 38, height: 38, shape: 'ellipse', 'background-color': sakura ? '#d95f88' : '#38bdf8', 'border-width': 2, 'border-color': sakura ? '#a83f67' : '#173946', color: sakura ? '#522539' : '#dce9ef', 'font-size': 10, 'font-weight': 600, 'text-valign': 'bottom', 'text-margin-y': 8, 'text-background-color': sakura ? '#fff8fb' : '#071219', 'text-background-opacity': .9, 'text-background-padding': 3, 'overlay-opacity': 0 } },
      ...
      { selector: 'edge', style: { width: 1.2, label: 'data(kind)', color: sakura ? '#8b6070' : '#66838f', 'font-size': 8, 'text-background-color': sakura ? '#fff8fb' : '#071219', 'text-background-opacity': .85, 'text-background-padding': 2, 'line-color': sakura ? '#d6a7b8' : '#365363', 'target-arrow-color': sakura ? '#d6a7b8' : '#365363', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier', opacity: .75 } }
    ] as any });
    cyRef.current = cy;
