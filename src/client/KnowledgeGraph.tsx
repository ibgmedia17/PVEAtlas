...
export function KnowledgeGraph({ model, rootId, onSelect }: { model: Topology; rootId: string; onSelect: (entity: Entity) => void }) {
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({ container: container.current, elements, minZoom: .2, maxZoom: 3, wheelSensitivity: .12, userPanningEnabled: true, userZoomingEnabled: true, autoungrabify: false, boxSelectionEnabled: false, style: [
      { selector: 'node', style: { label: 'data(displayLabel)', width: 38, height: 38, shape: 'ellipse', 'background-color': '#38bdf8', 'border-width': 2, 'border-color': '#173946', color: '#dce9ef', 'font-size': 10, 'font-weight': 600, 'text-valign': 'bottom', 'text-margin-y': 8, 'text-background-color': '#071219', 'text-background-opacity': .9, 'text-background-padding': 3, 'overlay-opacity': 0 } },
      { selector: 'node[focused="yes"]', style: { width: 58, height: 58, 'border-width': 4, 'border-color': '#e3f1f5', 'font-size': 12 } },
      { selector: 'node[expanded="yes"]', style: { 'border-color': '#2dd4bf' } },
      { selector: 'node[kind="guest"]', style: { 'background-color': '#2dd4bf', shape: 'round-rectangle' } },
      ...
      { selector: 'edge', style: { width: 1.2, label: 'data(kind)', color: '#66838f', 'font-size': 8, 'text-background-color': '#071219', 'text-background-opacity': .85, 'text-background-padding': 2, 'line-color': '#365363', 'target-arrow-color': '#365363', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier', opacity: .75 } }
    ] as any });
    cyRef.current = cy;
  ...
  }, [graph, expanded, onSelect, rootId]);
